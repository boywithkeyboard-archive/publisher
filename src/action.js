...
async function action() {
  const config = {
    ...
    mentionContributors: (0, import_core.getBooleanInput)("mention_contributors")
  };
  ...
  try {
    ...
    if (result.status === 200 && result.data instanceof Array && result.data.length > 0) {
  ...
  if (config.kind === "prepatch") {
    nextVersion += import_semver.default.prerelease(currentVersion) ? import_semver.default.inc(
      currentVersion,
      "prerelease",
      void 0,
...
`;
  ...
  if (config.mentionContributors) {
    ...
    arr[0] = arr[0].replace(", ", "");
    arr[arr.length - 1] = arr.length > 2 ? arr[arr.length - 1].replace(", ", ", & ") : arr[arr.length - 1].replace(", ", " & ");
    releaseBody += `
