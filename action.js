...
var require_command = __commonJS({
  'node_modules/@actions/core/lib/command.js'(exports) {
    ...
    function escapeData(s) {
      return utils_1.toCommandValue(s).replace(/%/g, '%25').replace(
        /\r/g,
        '%0D',
      ).replace(/\n/g, '%0A')
    }
...
function stringify(arr, offset = 0) {
  const uuid = (byteToHex[arr[offset + 0]] + byteToHex[arr[offset + 1]] +
...
var require_proxy = __commonJS({
  'node_modules/@actions/http-client/lib/proxy.js'(exports) {
    ...
    function getProxyUrl(reqUrl) {
      ...
      if (proxyVar) {
        try {
          ...
          if (
            !proxyVar.startsWith('http://') && !proxyVar.startsWith('https://')
          ) {
    ...
    function checkBypass(reqUrl) {
      ...
      for (
        const upperNoProxyItem of noProxy.split(',').map((x) =>
          x.trim().toUpperCase()
        ).filter((x) => x)
      ) {
        if (
          upperNoProxyItem === '*' ||
          upperReqHosts.some((x) =>
            x === upperNoProxyItem || x.endsWith(`.${upperNoProxyItem}`) ||
    ...
    function TunnelingAgent(options) {
      ...
      self.on('free', function onFree(socket, host, port, localAddress) {
        ...
        for (var i = 0, len = self.requests.length; i < len; ++i) {
          ...
          if (
            pending.host === options2.host && pending.port === options2.port
          ) {
...
var require_lib = __commonJS({
  'node_modules/@actions/http-client/lib/index.js'(exports) {
    ...
    var __awaiter = exports && exports.__awaiter ||
    ...
    var HttpClient = class {
      ...
      postJson(requestUrl, obj, additionalHeaders = {}) {
        return __awaiter(this, void 0, void 0, function* () {
          ...
          additionalHeaders[Headers.ContentType] = this
        ...
        return __awaiter(this, void 0, void 0, function* () {
          ...
          do {
            ...
            while (
              ...
              this._allowRedirects && redirectsRemaining > 0
            ) {
  ...
  'node_modules/@actions/core/lib/summary.js'(exports) {
    ...
    var Summary = class {
      ...
      wrap(tag, content, attrs = {}) {
        const htmlAttrs = Object.entries(attrs).map(([key, value]) =>
          ` ${key}="${value}"`
        ).join('')
        if (!content) {
...
var require_core = __commonJS({
  'node_modules/@actions/core/lib/core.js'(exports) {
    ...
    function addPath(inputPath) {
      ...
      process.env['PATH'] = `${inputPath}${path.delimiter}${
        process.env['PATH']
      }`
    }
...
var require_dist_node2 = __commonJS({
  'node_modules/@octokit/endpoint/dist-node/index.js'(exports) {
    ...
    function omit(object, keysToOmit) {
      return Object.keys(object).filter((option) =>
        !keysToOmit.includes(option)
      ).reduce((obj, key) => {
        obj[key] = object[key]
        return obj
      }, {})
        ...
        function (_, expression, literal) {
          if (expression) {
            ...
            if (operator && operator !== '+') {
              ...
              return (values.length !== 0 ? operator : '') +
    ...
    var VERSION = '6.0.12'
    var userAgent =
      `octokit-endpoint.js/${VERSION} ${universalUserAgent.getUserAgent()}`
    var DEFAULTS = {
      method: 'GET',
...
var require_mappingTable = __commonJS({
  'node_modules/tr46/lib/mappingTable.json'(exports, module2) {
    module2.exports = [
      ...
      [[65018, 65018], 'disallowed_STD3_mapped', [
        1589,
        1604,
        1609,
        32,
      ...
      [[65019, 65019], 'disallowed_STD3_mapped', [
        1580,
        1604,
        32,
        1580,
...
var require_tr46 = __commonJS({
  'node_modules/tr46/index.js'(exports, module2) {
    ...
    function validateLabel(label, processing_option) {
      ...
      var error = false
      if (
        normalize(label) !== label || label[3] === '-' && label[4] === '-' ||
        label[0] === '-' || label[label.length - 1] === '-' ||
        label.indexOf('.') !== -1 || label.search(combiningMarksRegex) === 0
      ) {
        error = true
      }
...
var require_url_state_machine = __commonJS({
  'node_modules/whatwg-url/lib/url-state-machine.js'(exports, module2) {
    ...
    function isDoubleDot(buffer) {
      ...
      return buffer === '..' || buffer === '%2e.' || buffer === '.%2e' ||
        buffer === '%2e%2e'
    }
    ...
    function isWindowsDriveLetterString(string) {
      return string.length === 2 && isASCIIAlpha(string.codePointAt(0)) &&
        (string[1] === ':' || string[1] === '|')
    }
    ...
    function utf8PercentDecode(str) {
      ...
      for (let i = 0; i < input.length; ++i) {
        if (input[i] !== 37) {
          ...
          input[i] === 37 && isASCIIHex(input[i + 1]) &&
    ...
    function parseIPv4Number(input) {
      let R = 10
      if (
        input.length >= 2 && input.charAt(0) === '0' &&
    ...
    function cannotHaveAUsernamePasswordPort(url) {
      return url.host === null || url.host === '' || url.cannotBeABaseURL ||
        url.scheme === 'file'
    }
    ...
    URLStateMachine.prototype['parse scheme'] = function parseScheme(c, cStr) {
      if (isASCIIAlphanumeric(c) || c === 43 || c === 45 || c === 46) {
        ...
        if (this.url.scheme === 'file') {
          ...
          isSpecial(this.url) && this.base !== null &&
        ...
        function parseHostName(c, cStr) {
          if (this.stateOverride && this.url.scheme === 'file') {
            ...
            if (isSpecial(this.url) && this.buffer === '') {
              ...
              this.stateOverride && this.buffer === '' &&
              (includesCredentials(this.url) || this.url.port !== null)
            ) {
              this.parseError = true
              return false
    ...
    URLStateMachine.prototype['parse port'] = function parsePort(c, cStr) {
      if (isASCIIDigit(c)) {
        ...
        isNaN(c) || c === 47 || c === 63 || c === 35 ||
        isSpecial(this.url) && c === 92 || this.stateOverride
      ) {
        if (this.buffer !== '') {
    ...
    URLStateMachine.prototype['parse path'] = function parsePath(c) {
      if (
        isNaN(c) || c === 47 || isSpecial(this.url) && c === 92 ||
        ...
        if (isDoubleDot(this.buffer)) {
          ...
          isSingleDot(this.buffer) && c !== 47 &&
          !(isSpecial(this.url) && c === 92)
        ) {
          this.url.path.push('')
        } else if (!isSingleDot(this.buffer)) {
          if (
            this.url.scheme === 'file' && this.url.path.length === 0 &&
        ...
        this.buffer = ''
        if (
          this.url.scheme === 'file' && (c === void 0 || c === 63 || c === 35)
        ) {
    ...
    URLStateMachine.prototype['parse cannot-be-a-base-URL path'] =
      function parseCannotBeABaseURLPath(c) {
        if (c === 63) {
          ...
          if (!isNaN(c)) {
            this.url.path[0] = this.url.path[0] +
    ...
    URLStateMachine.prototype['parse query'] = function parseQuery(c, cStr) {
      if (isNaN(c) || !this.stateOverride && c === 35) {
        if (
          !isSpecial(this.url) || this.url.scheme === 'ws' ||
          this.url.scheme === 'wss'
        ) {
        ...
        for (let i = 0; i < buffer.length; ++i) {
          if (
            buffer[i] < 33 || buffer[i] > 126 || buffer[i] === 34 ||
            buffer[i] === 35 || buffer[i] === 60 || buffer[i] === 62
          ) {
  ...
  'node_modules/node-fetch/lib/index.js'(exports, module2) {
    ...
    var Blob = class _Blob {
      constructor() {
        ...
        let type = options && options.type !== void 0 &&
    ...
    function Body(body) {
      ...
      if (body instanceof Stream) {
        body.on('error', function (err) {
          const error = err.name === 'AbortError' ? err : new FetchError(
    ...
    function convertBody(buffer, headers) {
      ...
      if (!res && str) {
        res =
          /<meta[\s]+?http-equiv=(['"])content-type\1[\s]+?content=(['"])(.+?)\2/i
            .exec(str)
        if (!res) {
          res =
            /<meta[\s]+?content=(['"])(.+?)\1[\s]+?http-equiv=(['"])content-type\3/i
              .exec(str)
          if (res) {
            res.pop()
          }
    ...
    function isURLSearchParams(obj) {
      if (
        typeof obj !== 'object' || typeof obj.append !== 'function' ||
        typeof obj.delete !== 'function' || typeof obj.get !== 'function' ||
        typeof obj.getAll !== 'function' || typeof obj.has !== 'function' ||
    ...
    function isBlob(obj) {
      return typeof obj === 'object' && typeof obj.arrayBuffer === 'function' &&
        typeof obj.type === 'string' && typeof obj.stream === 'function' &&
    ...
    var Headers = class _Headers {
       ...
       */
      constructor() {
        let init = arguments.length > 0 && arguments[0] !== void 0
          ? arguments[0]
          : void 0
    ...
    function getHeaders(headers) {
      let kind = arguments.length > 1 && arguments[1] !== void 0
        ? arguments[1]
        : 'key+value'
    ...
    var Response = class _Response {
      constructor() {
        let body = arguments.length > 0 && arguments[0] !== void 0
          ? arguments[0]
          : null
    ...
    function isAbortSignal(signal) {
      const proto = signal && typeof signal === 'object' &&
        Object.getPrototypeOf(signal)
      return !!(proto && proto.constructor.name === 'AbortSignal')
    }
    var Request = class _Request {
      constructor(input) {
        let init = arguments.length > 1 && arguments[1] !== void 0
          ? arguments[1]
          : {}
        let parsedURL
        if (!isRequest(input)) {
        ...
        this.follow = init.follow !== void 0
          ? init.follow
          : input.follow !== void 0
          ? input.follow
          : 20
        this.compress = init.compress !== void 0
          ? init.compress
    ...
    function getNodeRequestOptions(request) {
      ...
      if (
        request.signal && request.body instanceof Stream.Readable &&
    ...
    function fetch(url, opts) {
      ...
      return new fetch.Promise(function (resolve, reject) {
        ...
        if (parseInt(process.version.substring(1)) < 14) {
          req.on('socket', function (s) {
            s.addListener('close', function (hadError) {
              ...
              if (
                response && hasDataListener && !hadError &&
        ...
        req.on('response', function (res) {
          ...
          if (fetch.isRedirect(res.statusCode)) {
            ...
            switch (request.redirect) {
              ...
              case 'follow':
                ...
                if (
                  res.statusCode !== 303 && request.body &&
          ...
          if (
            !request.compress || request.method === 'HEAD' ||
            codings === null || res.statusCode === 204 || res.statusCode === 304
          ) {
          ...
          if (
            codings == 'br' && typeof zlib.createBrotliDecompress === 'function'
          ) {
...
var require_dist_node5 = __commonJS({
  'node_modules/@octokit/request/dist-node/index.js'(exports) {
    ...
    function toErrorMessage(data) {
      ...
      if ('message' in data) {
        if (Array.isArray(data.errors)) {
          return `${data.message}: ${
            data.errors.map(JSON.stringify).join(', ')
          }`
        }
        return data.message
      }
    ...
    var request = withDefaults(endpoint.endpoint, {
      headers: {
        'user-agent':
          `octokit-request.js/${VERSION} ${universalUserAgent.getUserAgent()}`,
      },
    })
    exports.request = request
  },
...
` + data.errors.map((e) => ` - ${e.message}`).join('\n')
    ...
    function graphql(request2, query, options) {
      ...
      const baseUrl = parsedOptions.baseUrl ||
        request2.endpoint.DEFAULTS.baseUrl
    ...
    var graphql$1 = withDefaults(request.request, {
      headers: {
        'user-agent':
          `octokit-graphql.js/${VERSION} ${universalUserAgent.getUserAgent()}`,
      },
      method: 'POST',
      url: '/graphql',
    })
    ...
    function decorate(octokit, scope, methodName, defaults, decorations) {
      ...
      function withDecorations(...args) {
        ...
        if (decorations.renamedParameters) {
          ...
          for (
            const [name, alias] of Object.entries(decorations.renamedParameters)
          ) {
            if (name in options2) {
              octokit.log.warn(
...
var require_dist_node10 = __commonJS({
  'node_modules/@octokit/plugin-paginate-rest/dist-node/index.js'(exports) {
    ...
    function normalizePaginatedListResponse(response) {
      ...
      const responseNeedsNormalization = 'total_count' in response.data &&
    ...
    function iterator(octokit, route, parameters) {
      ...
      const requestMethod = typeof route === 'function'
        ? route
        : octokit.request
  ...
  'node_modules/semver/internal/re.js'(exports, module2) {
    ...
    var makeSafeRegex = (value) => {
      for (const [token, max] of safeRegexReplacements) {
        value = value.split(`${token}*`).join(`${token}{0,${max}}`).split(
          `${token}+`,
        ).join(`${token}{1,${max}}`)
      }
      return value
  ...
  'node_modules/semver/classes/semver.js'(exports, module2) {
    ...
    var SemVer = class _SemVer {
      constructor(version2, options) {
        ...
        const m = version2.trim().match(
          options.loose ? re[t.LOOSE] : re[t.FULL],
        )
        if (!m) {
    ...
    var coerce = (version2, options) => {
      ...
      if (!options.rtl) {
        ...
        while (
          ...
          re[t.COERCERTL].lastIndex = next.index + next[1].length +
            next[2].length
        }
  ...
  'node_modules/lru-cache/index.js'(exports, module2) {
    ...
    var LRUCache = class {
      constructor(options) {
        ...
        if (
          options.max && (typeof options.max !== 'number' || options.max < 0)
        ) {
  ...
  'node_modules/semver/classes/range.js'(exports, module2) {
    var Range = class _Range {
      constructor(range, options) {
        ...
        this.set = this.raw.split('||').map((r) => this.parseRange(r.trim()))
          .filter((c) => c.length)
        if (!this.set.length) {
      ...
      format() {
        this.range = this.set.map((comps) => comps.join(' ').trim()).join('||')
          .trim()
        return this.range
      }
      toString() {
      ...
      parseRange(range) {
        ...
        let rangeList = range.split(' ').map((comp) =>
          parseComparator(comp, this.options)
        ).join(' ').split(/\s+/).map((comp) => replaceGTE0(comp, this.options))
        if (loose) {
        ...
        const comparators = rangeList.map((comp) =>
          new Comparator(comp, this.options)
        )
  ...
  'node_modules/semver/ranges/to-comparators.js'(exports, module2) {
    ...
    var toComparators = (range, options) =>
      new Range(range, options).set.map((comp) =>
        comp.map((c) => c.value).join(' ').trim().split(' ')
      )
...
async function action() {
  const config = {
    ...
    mentionContributors: (0, import_core.getBooleanInput)(
  ...
  if (config.kind === 'prepatch') {
    nextVersion += import_semver.default.prerelease(currentVersion)
      ? import_semver.default.inc(
        currentVersion,
        'prerelease',
        void 0,
    ...
    nextVersion += import_semver.default.inc(currentVersion, 'patch')
    ...
    nextVersion += import_semver.default.inc(currentVersion, 'minor')
    ...
    nextVersion += import_semver.default.inc(currentVersion, 'major')
  } else {
...
`
  ...
  for (const { user, merged_at, number, body, merge_commit_sha } of data) {
    ...
    const linkifyReferences = (commit) => {
      ...
      for (const m of matches) {
        commit = commit.replace(
          `(${m})`,
          `([${m}](https://github.com/${import_github.context.repo.owner}/${import_github.context.repo.repo}/pull/${
            m.slice(1)
          }))`,
        )
      }
    ...
    if (
      comments.length > 0 && comments.some(
        (c2) =>
          c2.body !== void 0 && c2.body === '?log ignore' &&
          (c2.author_association === 'COLLABORATOR' ||
            c2.author_association === 'MEMBER' ||
            c2.author_association === 'OWNER'),
      )
    ) {
      continue
    }
...
Contributed by ${arr.join('')}`
  ...
  ;(0, import_core.setOutput)('tag_name', nextVersion)
